...
public class PriorityQueue<K, V extends Comparable<? super V>>
{
	...
	for (int i = 0; i < itemTable.size(); i++)
	    {
		int hi = itemTable.get(i).heapIndex;
		if (itemHeap.get(hi).tableIndex != i)
		    {
			System.err.println("PQ.validate: bad pointers for " + itemTable.get(i) + " " + itemHeap.get(hi));
		    }

		K item = itemTable.get(i).item;
		int ti = itemMap.get(item);
		if (ti != i)
		    {
			System.err.println("PQ.validate: bad map entry for " + item + " " + itemTable.get(i) + " " + itemTable.get(ti));
		    }
	    }

	for (int i = 0; i < itemHeap.size(); i++)
	    {
		int ti = itemHeap.get(i).tableIndex;
		if (itemTable.get(ti).heapIndex != i)
		    {
			System.err.println("PQ.validate: bad pointers from heap: " + itemHeap.get(i) + " " + itemTable.get(ti));
		    }
	    }

	for (Map.Entry<K, Integer> e : itemMap.entrySet())
	    {
		if (e.getValue() >= itemTable.size())
		    {
			System.err.println("Extra entry in map: " + e);
		    }
	    }
    }
	...
	//	System.err.println("PQ.remove: " + result);
    ...
    {
	//	System.err.println("PQ.changePriority: " + item + pri);
    ...
    {
	//	System.err.println("PQ.decreasePriority: " + item + " " + pri);
